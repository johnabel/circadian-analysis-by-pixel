...
def PlotOptions(uselatex=False, ticks='out'):
    ...
    # colors: 
    # POSTDOC
    labels = ['f', 'h', 'i', 'j', 'l', 'm', 'n']
    med_colors = ['#4477AA', '#EE6677', '#228833', '#CCBB44', '#66CCEE', '#AA3377', '#BBBBBB']
    li_colors = ['#BBCCEE', '#FFCCCC', '#CCDDAA', '#EEEEBB', '#CCEEFF', '', '#DDDDDD']
    da_colors = ['#222255', '#663333', '#225522', '#666633', '#225555', '', '#555555']
    for i in range(7):
        matplotlib.colors.ColorConverter.colors[labels[i]] = med_colors[i]
        if i !=5:
            matplotlib.colors.ColorConverter.colors[labels[i]+'l'] = li_colors[i]
            matplotlib.colors.ColorConverter.colors[labels[i]+'d'] = da_colors[i]
